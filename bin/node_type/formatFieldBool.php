...
if (!class_exists("commandFormatFieldBool")) {
    class commandFormatFieldBool extends driverCommand {
            ...
            if ($p["toread"] == $p["towrite"]) {
                ...
                if ($p["multivalued"]) {
                    ...
                    if ($p["toread"] || $p["readonly"]) { // to read
                        ...
                        echo $p["label"];
                        echo '</label>';
                        echo '<div id="'.$p["fieldname"].'">'.($p["value"]?"Yes":"No").'</div>';
                        echo '</div>';
                        echo '</div>';
                    } else { // to write
                        if ($p["value"] == "") {
                        ...
                        echo '<input type="checkbox" name="'.$p["fieldname"].'" id="'.$p["fieldname"].'" value="1" '.($p["value"]?"checked":"").'>';
                        echo $p["label"];
                        echo '</label>';
...
EOT;
        ...
        public static function getHelp() {
            return array(
                "description" => "Format boolean field to read or write.", 
                "parameters" => array(
...
return new commandFormatFieldBool();