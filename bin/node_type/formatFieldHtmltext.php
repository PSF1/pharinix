...
if (!class_exists("commandFormatFieldHtmltext")) {
    class commandFormatFieldHtmltext extends driverCommand {
                ...
                if ($p["multivalued"]) {
                    ...
                    if ($p["toread"] || $p["readonly"]) { // to read
                        ...
                        if (self::$firstHtmltextWrite) {
                            ...
                            echo '<script type="text/javascript" src="'.CMS_DEFAULT_URL_BASE.'/usr/ckeditor/ckeditor.js"></script>';
                        ...
                        echo '</label>';
                        echo '<textarea id="'.$p["fieldname"].'" name="'.$p["fieldname"].'" class="form-control ckeditor" '.($p["required"]?"required":"").'>'.$p["value"].'</textarea>';
                        echo "<div class=\"help help-block\">".$p["help"]."</div>";
                        echo '</div>';
                        echo '</div>';
                        $js = <<<EOT
//TODO: At submit change codification to base64.
EOT;
        ...
        public static function getHelp() {
            return array(
                "package" => 'core',
                "description" => __("Format HTML text field to read or write."), 
                "parameters" => array(
...
return new commandFormatFieldHtmltext();