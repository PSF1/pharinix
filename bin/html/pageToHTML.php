    ...
    function pageToHTMLParseBlock($pageId, $blk) {
        foreach ($blk as $key => $rows) {
            if ($key != '@attributes') {
                foreach($rows as $row) {
                    echo "<div class=\"row\">";
                    if (CMS_DEBUG) echo "<h6><span class=\"label label-success\">row ID: ".$row['@attributes']["id"]."</span></h6>";
                    foreach($row["col"] as $col) {
                        ...
                        echo ">";
                        if (CMS_DEBUG) echo "<h6><span class=\"label label-success\">Col ID: ".$col['@attributes']["id"]."</span></h6>";
...
if ($def !== false) {
    if (is_file($def->fields["template"])) {
        ...
        echo '<head>';
            ...
            if ($charset != "") echo $charset;
            if (isset($struct["page"][0]["title"][0])) echo '<title>'.$struct["page"][0]["title"][0].'</title>';
        ...
        if (CMS_DEBUG) echo "<h6><span class=\"label label-success\">Body</span></h6>";
        pageToHTMLParseBlock($def->fields["id"], $struct["page"][0]["body"][0]);
        echo "</div>";
        echo '<div id="footer">';
        echo '<div class="container-fluid">';
        if (CMS_DEBUG) echo "<h6><span class=\"label label-success\">Foot</span></h6>";
